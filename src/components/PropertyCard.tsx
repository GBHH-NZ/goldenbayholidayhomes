...
export function PropertyCard({
  ...
  return (
    <article
      className={clsx(
        "flex flex-col overflow-hidden rounded-md border border-drift/70 bg-sand",
        open && "sm:col-span-2",
      )}
    >
      {canEmbed ? (
        <button
          ...
          className="relative block aspect-[4/3] w-full cursor-pointer overflow-hidden bg-drift focus:outline-none focus-visible:ring-2 focus-visible:ring-sea"
        ...
        <a
          ...
          aria-label={`Book ${home.shortTitle || home.title}`}
          className="relative block aspect-[4/3] overflow-hidden bg-drift focus:outline-none focus-visible:ring-2 focus-visible:ring-sea"
          onMouseEnter={() => {
            if (canHoverGallery) setPhotoIndex(1);
          }}
          onMouseLeave={() => setPhotoIndex(0)}
...
export function PropertyGrid({
  ...
  return (
    <div className={clsx("grid gap-6 sm:grid-cols-2", className)}>
      {homes.map((h) => (
        <PropertyCard
          key={h.slug}
          home={h}
