...
|------|----------|-------------------|-------------|
| `NEXT_PUBLIC_SITE_URL` | Recommended | `https://goldenbayholidayhomes.nz` | Canonical site URL for metadata, sitemap, OG |
| `GUESTY_API_BASE` | Optional | `https://booking-api.guesty.com` | Guesty Booking Engine API base |
| `BASE_PATH` | Recommended for custom domain | `none` | Empty asset prefix for apex/www DNS → Pages. Set `/goldenbayholidayhomes` only for project-URL preview on `*.github.io` |

**Custom domain (current):** set `BASE_PATH=none` and `NEXT_PUBLIC_SITE_URL=https://goldenbayholidayhomes.nz`. Deploy builds assets at the domain root so CSS/JS load on `https://goldenbayholidayhomes.nz/`.
...
Apex is the canonical host. GitHub will redirect `www` → apex once both DNS records exist.
...
2. **Custom domain** → `goldenbayholidayhomes.nz` → Enforce HTTPS (after DNS propagates and the certificate is issued)
...
4. `public/CNAME` ships `goldenbayholidayhomes.nz` into the static export (must match the Pages custom domain)
5. After go-live: Google Search Console → add/verify `https://goldenbayholidayhomes.nz` → submit `https://goldenbayholidayhomes.nz/sitemap.xml`

If HTTPS stays on the default `*.github.io` certificate: remove the custom domain, save, re-add `goldenbayholidayhomes.nz`, then wait for **Enforce HTTPS**.
...
env:
  ...
  NEXT_PUBLIC_SITE_URL: ${{ vars.NEXT_PUBLIC_SITE_URL || 'https://goldenbayholidayhomes.nz' }}
```
